...
function displayFunction() {
	{
		...
		for (var ord = 1; ord <= order; ord++) {
			if (ord == 1) {
				equ +=
					"(" + cParam[ord][0].toExponential(3).toString() + ")X +";
			} else {
				equ +=
					"(" +
					cParam[ord][0].toExponential(3).toString() +
					")X^" +
					ord.toString() +
					"+";
			}
		...
		equ +=
			"(" +
			(cParam[0][0] - (width - height) - 50).toExponential(3).toString() +
			")";
	...
	try {
		...
		for (var x = 0; x < width - 50; x++) {
			...
			if (true) {
				...
				line(
					x + 50,
					height - 50 - y + 50 + (width - height),
					x + 50 + 1,
					height - 50 - yNext + 50 + (width - height)
				);
			}
		}
	} catch (err) {}
